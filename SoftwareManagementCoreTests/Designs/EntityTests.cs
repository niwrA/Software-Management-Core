...
namespace SoftwareManagementCoreTests.Designs
{
  ...
  public class EntityTests
  {
    ...
    [Fact(DisplayName = "Can Change PluralName for EntityElement")]
    public void CanChangePluralNameEntityElement()
    {
      var stateMock = new Mock<IEntityElementState>();
      var repoMock = new Mock<IDesignStateRepository>();
      var sut = new EntityElement(stateMock.Object, repoMock.Object);

      sut.ChangePluralName("new", "old"); // todo: should be mock?

      stateMock.Setup(s => s.PluralName).Returns("old");
      sut.ChangePluralName("new", "old");
      stateMock.VerifySet(t => t.PluralName = "new");
    }

    [Fact(DisplayName = "Can Change IsCollection of EntityElement")]
    public void CanChangeIsCollectionForEntityElement()
    {
      var stateMock = new Mock<IEntityElementState>();
      var repoMock = new Mock<IDesignStateRepository>();
      var sut = new EntityElement(stateMock.Object, repoMock.Object);
      Assert.False(sut.IsCollection);

      sut.ChangeIsCollection(true);

      stateMock.VerifySet(t => t.IsCollection = true);
    }
